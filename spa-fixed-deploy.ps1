...
try {
    ...
    firebase firestore:databases:create --project ar-tracker-c226f radiusdb --location=us-central
...
# Parse json manually to avoid errors in ConvertFrom-Json
$firebaseJson = $firebaseJsonContent | ConvertFrom-Json

# Update rewrites
$firebaseJson.hosting | Add-Member -MemberType NoteProperty -Name "rewrites" -Value @(
    @{
        source = "/api/**"
        function = "api" 
    },
...
if (-not $firebaseJson.firestore.database) {
    $firebaseJson.firestore | Add-Member -MemberType NoteProperty -Name "database" -Value "radiusdb" -Force
}
...
if (Test-Path $indexHtmlPath) {
    ...
    Write-Host "Warning: index.html not found at $indexHtmlPath" -ForegroundColor Red
}
